	...
	else
	{
		...
		if (found != 0 && found != ins.m_NetList)
		{
			...
			while (OldNetList != m_nets[OldList].end())
			{
				...
				//
				// Bugfix contributed by Chunk.Lee (see TinyCAD Developer Discussions on TinyCAD web site dated 2013-12-18
				// BUG fixed: "(*OldNetList).m_a" has no value if (OldNetList) generated by a power pin!!
				if ((*OldNetList).m_a.hasValue()) m_nodes[(*OldNetList).m_a] = NewList;
				++OldNetList;
			}
...
void getFileNameFromPath(const TCHAR *path, TCHAR *filename, int max_size){
	const TCHAR* filename_plus_ext = _tcsrchr(path, '\\');
	int size; // Number of charaters of the file name

	if (filename_plus_ext == NULL) filename_plus_ext = filename;
	filename_plus_ext++;

	const TCHAR* ext = _tcsrchr(filename_plus_ext, '.');
	if (ext == NULL) size = _tcslen(filename_plus_ext);
	else size = ext - filename_plus_ext;

	size = min(max_size - 1, size);  // maximum file name size

	_tcsncpy_s(filename, max_size, filename_plus_ext, size);
	filename[size] = 0;
}
...
void CNetList::componentsBegin(CTinyCadMultiDoc *p)
{
	...
	m_ComponentIterator.fi = m_imports.begin();
	...
	m_ComponentIterator.drawing = m_ComponentIterator.dsn->GetSheet(m_ComponentIterator.sheet)->GetDrawingBegin();

	if ((*m_ComponentIterator.drawing)->GetType() != xMethodEx3) componentsNext();
}

void CNetList::componentsNext()
{
	do {
		m_ComponentIterator.drawing++;
		if (m_ComponentIterator.drawing == m_ComponentIterator.dsn->GetSheet(m_ComponentIterator.sheet)->GetDrawingEnd())
		{
			// last drawing, go to next sheet
			m_ComponentIterator.sheet++;

			if (m_ComponentIterator.sheet == m_ComponentIterator.dsn->GetNumberOfSheets())
			{
				// last sheet go to the next file
				m_ComponentIterator.sheet = 0;
				m_ComponentIterator.fi++;
				if (m_ComponentIterator.fi == m_imports.end())
				{
					// last file, fininsh
					m_ComponentIterator.end = true;
				}
				else
				{
					if ((*m_ComponentIterator.fi)->getFileNameIndex() == 0)
						// for file 0 use the current document
						m_ComponentIterator.dsn = m_ComponentIterator.pDesign;
					else
					{
						m_ComponentIterator.dsn = static_cast<CTinyCadMultiDoc *> ((*m_ComponentIterator.fi)->getDesign());
			...
			if (!m_ComponentIterator.end) m_ComponentIterator.drawing =
				m_ComponentIterator.dsn->GetSheet(m_ComponentIterator.sheet)->GetDrawingBegin();

		}
	} while (!m_ComponentIterator.end && (*m_ComponentIterator.drawing)->GetType() != xMethodEx3);
}

CDrawMethod *CNetList::currentComponent()
{
	return dynamic_cast<CDrawMethod *>(*m_ComponentIterator.drawing);
}

bool CNetList::componentsEnd()
{
	return m_ComponentIterator.end;
}

Bus_Properties::Bus_Properties(CString &ref, bus_type t){
	LPCTSTR str = (LPCTSTR)ref;
	enum {in_name, in_start, in_end, after_end} position = in_name;
	int start_of_start = 0, start_of_end = 0;
	type = t;
	...
	for (int i=0; *str != 0; str++, i++) {
		...
		case in_start:
			if (c == ':') {
				start = _ttoi(ref.Mid(start_of_start, i - start_of_start));
				start_of_end = i + 1;
				position = in_end;
			}
			else if (c == ')') {
		...
		case in_end:
			if (c == ')') {
				end = _ttoi(ref.Mid(start_of_end, i - start_of_end));
				position = after_end;
			}
		}
	}
...
void print_signal_dec(CString &str, Bus_Properties bus){
	// Prints the declaration of a signal
	CString aux;
	
	TCHAR *in_or_out=_T("");
	switch (bus.type) {
	case Bus_Properties::internal:
		in_or_out = _T("");
		break;
	case Bus_Properties::input:
		in_or_out = _T("in");
		break;
	case Bus_Properties::output:
		in_or_out = _T("out");
		break;
	}
	...
	if (bus.is_bus) {
		aux.Format(_T("%s: %s std_logic_vector(%d downto %d)"), (LPCTSTR) bus.name, in_or_out, bus.start, bus.end);
	}
	else {
		aux.Format(_T("%s: %s std_logic"), (LPCTSTR) bus.name, in_or_out);
	}

	str.Append(aux);
}

void _ftprintf_signal_dec(FILE *theFile, Bus_Properties bus){
	CString str;
	print_signal_dec(str, bus);
	_ftprintf(theFile, str);
}

void _ftprintf_signal_dec(FILE *theFile, CString ref, Bus_Properties::bus_type t){
	Bus_Properties bus(ref, t);
	_ftprintf_signal_dec(theFile, bus);
}

void _ftprintf_signal_ref(FILE *theFile, CString ref){
	Bus_Properties bus(ref);
	CString aux;
	if (bus.is_bus) {
		if (bus.start == bus.end)  _ftprintf(theFile, _T("%s(%d)"), (LPCTSTR)bus.name, bus.start);
		else _ftprintf(theFile, _T("%s(%d downto %d)"), (LPCTSTR)bus.name, bus.start, bus.end);
	}
	else {
		_ftprintf(theFile, bus.name);
	}
}

// Class Signals stores all the signals in the design accessed by signal name.
class Signals : public std::map < CString, Bus_Properties > {
public:
	void add(Bus_Properties bus){
		if (find(bus.name) == this->end()) {
			operator[](bus.name) = bus;
		}
		else {
			Bus_Properties &b = at(bus.name);
			b.start = max(bus.start, b.start);
			b.end = min(bus.end, b.end);
		}
	}
};
...
void CNetList::WriteVHDLFile(CTinyCadMultiDoc *pDesign, const TCHAR *path)
{
	FILE *theFile;
	errno_t err;
	TCHAR short_filename[255];
	CString str;

	// The design is formed by component connected through nets.
	// The components have references and have names (types).
	// Nets have labels formed by a name and bus properties.
	// Nets with the same name form a signal.
	// Signals also have a label string.
	
	// Data Structures
	
	// To write the entity declaration

	// none (simply searches all the components for input or output ports).

	// To write the components

	std::set<CString> referenced_components;	// Keep track of the components that we have outputed (referenced) 
	Signals signals;							// Stores all the signals in the design accessed by signal name

	std::map<CString, CString> map_reference_name;		// A Map of the reference to the name of the component

	typedef std::map<CString, std::map<CString, int>> map_pin_number_to_pos_type;
	map_pin_number_to_pos_type map_pin_number_to_pos;	// A Map of the pin number of a component to its positon on the port map

	// A Map of the pin number of a component to its name
	std::map<CString, std::map<CString, CString>> map_pin_number_to_name;
	...
	typedef std::map<int, CString> t_single_portmap;	// Gives the net label given the position
	typedef std::map<CString, t_single_portmap> t_all_portmaps;

	t_all_portmaps portmaps;			// Maps the reference and the position to the signal label

	// code
	
	err = _tfopen_s(&theFile, path, _T("w"));
	if ((theFile == NULL) || (err != 0))
	{
		Message(IDS_CANNOTOPEN);
		return;
	...
	bool first = true;
	for (componentsBegin(pDesign); !componentsEnd(); componentsNext()) {

		CDrawMethod *pMethod = currentComponent();
		if (currentComponent()->GetName().Compare(_T("INPUT")) == 0) {

			if (!first) str.Append(_T(";\n\t\t"));
			else first = false;

			Bus_Properties bus(pMethod->GetRef(), Bus_Properties::input);
			print_signal_dec(str, bus);
			signals.add(bus);
		}
	}
	...
	for (componentsBegin(pDesign); !componentsEnd(); componentsNext()) {
		...
		CDrawMethod *pMethod = currentComponent();
		if (currentComponent()->GetName().Compare(_T("OUTPUT")) == 0) {

			if (!first) str.Append(_T(";\n\t\t"));
			else first = false;

			Bus_Properties bus(pMethod->GetRef(), Bus_Properties::output);
			print_signal_dec(str, bus);
			signals.add(bus);
		}
	}

	if (!str.IsEmpty()) { // If there are any inputs or outputs
		_ftprintf(theFile, _T("\tport(\t"));
		_ftprintf(theFile, str);
		_ftprintf(theFile, _T(");\n"));
	}

	_ftprintf(theFile, _T("end %s;\n"), short_filename);
	...
	for (componentsBegin(pDesign); !componentsEnd(); componentsNext()) {
		...
		map_reference_name[pMethod->GetRef()] = Name;

		// Do we need to output this component?
		if (referenced_components.find(Name) == referenced_components.end() && Name.Compare(_T("INPUT")) != 0 && Name.Compare(_T("OUTPUT")) != 0)
		{
			referenced_components.insert(Name);

			_ftprintf(theFile, _T("\tcomponent %s\n \t\tport(\t"), (LPCTSTR) Name);

			// Get the pins.
			// These are DrawPin objects in the drawing collection of the symbol.
			CDPoint tr;
			drawingCollection method;
			pMethod->ExtractSymbol(tr, method);

			int position = 0;
			// Writes the input pins
			bool first_pin = true;
			for (drawingIterator i = method.begin(); i != method.end(); ++i){
				if ((*i)->GetType() == xPinEx) {
					CDrawPin *pin = dynamic_cast<CDrawPin *>(*i);

					if (pin->GetElec() == 0) // Input pin (see GetElectricalTypeName)
					{

						if (!first_pin) _ftprintf(theFile, _T(";\n\t\t\t"));
						else first_pin = false;

						if (map_pin_number_to_pos[pMethod->GetName()].count(pin->GetNumber()) > 0)
							_ftprintf(theFile, _T("ERROR: Duplicated pin number, %s pin number %s\n"),
							(LPCTSTR) pMethod->GetName(), (LPCTSTR) pin->GetNumber());
						
						_ftprintf_signal_dec(theFile, pin->GetPinName(), Bus_Properties::input);
						map_pin_number_to_pos[pMethod->GetName()][pin->GetNumber()] = position++;
						map_pin_number_to_name[pMethod->GetName()][pin->GetNumber()] = pin->GetPinName();
					}
				}
			}

			// Writes the output pins
			for (drawingIterator i = method.begin(); i != method.end(); ++i){
				if ((*i)->GetType() == xPinEx) {
					CDrawPin *pin = dynamic_cast<CDrawPin *>(*i);

					if (pin->GetElec() == 1) // Ouput pin (see GetElectricalTypeName)
					{
						...
						_ftprintf_signal_dec(theFile, pin->GetPinName(), Bus_Properties::output);
						map_pin_number_to_pos[pMethod->GetName()][pin->GetNumber()] = position++;
						map_pin_number_to_name[pMethod->GetName()][pin->GetNumber()] = pin->GetPinName();
					}
				}
	...
	// m_nets is a map of <int,vector<CNetListNode>>
	// (*nit).second is the second element of the map, so its a vector
	...
	while (nit != m_nets.end())
	{
		...
		// If it finds pins use to size the bus!
		while (nv_it != (*nit).second.end())
		{
			CNetListNode& theNode = *nv_it;
			++nv_it;
			...
			if (!theNode.getLabel().IsEmpty() && !Labeled)
			{
				...
				if (!bus_size) bus_size = bus.start - bus.end + 1;
				else if (bus_size != bus.start - bus.end + 1)
					_ftprintf(theFile, _T("ERROR: Different BUS sizes at label %s\n"), (LPCTSTR) theLabel);
			}
			...
			if (m != NULL) {
				if (m->GetName().Compare(_T("INPUT")) == 0 || m->GetName().Compare(_T("OUTPUT")) == 0)
				{
					...
					if (!bus_size) bus_size = bus.start - bus.end + 1;
						...
						_ftprintf(theFile, _T("ERROR: Different BUS sizes at IN/OUT %s\n"), (LPCTSTR) theLabel);

				} else
				if (!theNode.m_pin.IsEmpty()) {
					CString &pin_name = map_pin_number_to_name[m->GetName()][theNode.m_pin];
					Bus_Properties bus(pin_name);
					if (!bus_size) bus_size = bus.start - bus.end + 1;
					else if (bus_size != bus.start - bus.end + 1)
						_ftprintf(theFile, _T("ERROR: Different BUS sizes: %s pin name %s, pin number %s\n"), 
						(LPCTSTR) m->GetRef(), (LPCTSTR) pin_name, (LPCTSTR) theNode.m_pin);
				}
			}

			if (!theNode.m_reference.IsEmpty()) ++count;
		}
		...
		if (!in_out) {
			Bus_Properties bus(theLabel, Bus_Properties::internal);
			signals.add(bus);
		}
	...
	for (Signals::iterator i = signals.begin(); i != signals.end(); i++){
		Bus_Properties &bus = (*i).second;
		if (bus.type == Bus_Properties::internal) {
			_ftprintf(theFile, _T("\tsignal "));
			_ftprintf_signal_dec(theFile, bus);
			_ftprintf(theFile, _T(";\n"));
		}
	...
	while (nit != m_nets.end())
	{
		...
		CString &theLabel = signal_labels[(*nit).first];
		...
		while (nv_it != (*nit).second.end())
		{
			...
			if (m != NULL)
				if (m->GetName().Compare(_T("INPUT")) != 0 && m->GetName().Compare(_T("OUTPUT")) != 0)
					if (!theNode.m_reference.IsEmpty()) {
						portmaps[theNode.m_reference][map_pin_number_to_pos[m->GetName()][theNode.m_pin]] = theLabel;
					}
		}
	...
	// prints to the file the port map of all the component
	for (t_all_portmaps::iterator it = portmaps.begin(); it != portmaps.end(); it++){
		_ftprintf(theFile, _T("\t%s: %s port map ("), (LPCTSTR) it->first, (LPCTSTR) map_reference_name[it->first]);
		bool first = true;
		for (t_single_portmap::iterator signal_it = it->second.begin(); signal_it != it->second.end(); signal_it++){
			if (first) first = false;
			else _ftprintf(theFile, _T(", "));

			CString the_label = signal_it->second;
			
			// Use just the signal name if adequate
			Bus_Properties bus(the_label);
			if (bus.is_bus) {
				Bus_Properties &signal_bus = signals[bus.name];
				if (bus.start == signal_bus.start && bus.end == signal_bus.end)
					the_label = bus.name;
			}

			_ftprintf_signal_ref(theFile, the_label);	// Transforms labels to VHDL
		}

		_ftprintf(theFile, _T(");\n"));
	}

	_ftprintf(theFile, _T("end structural;\n"));
