...
CString CDrawMethod::Find(const TCHAR *theSearchString)
{
	...
	for (unsigned int lp = 0; lp < m_fields.size(); lp++)
	{
		...
		if (HoldString.Find(theSearchString) != -1) return m_fields[lp].m_value;
	}
...
int CDrawMethod::GetFieldIndexByName(CString name, bool caseSensitive)
{
	...
	for (i = 0, it = m_fields.begin(); it != m_fields.end(); ++it, ++i)
	{
		CField& field = *it;
...
CString CDrawMethod::GetFieldByName(CString name, bool caseSensitive)
{
	...
	for (fieldCollection::iterator it = m_fields.begin(); it != m_fields.end(); ++it)
	{
		CField& field = *it;
...
{ //Get the value of the field with the field name and other decoration optionally prepended onto it
	...
	if (!m_fields[which].m_show)
	{ //Whether to show this field overrides conditional controls over how to show the field
		r = _T("");
	}
	else
	{
...
void CDrawMethod::OldLoad(CStream &archive)
{
	...
	archive >> SymName >> m_fields[Name].m_value >> m_fields[Other].m_value >> m_fields[Ref].m_value;
...
void CDrawMethod::OldLoad2(CStream &archive)
{
	...
	archive >> SymName >> m_fields[Name].m_value >> m_fields[Other].m_value >> m_fields[Ref].m_value >> attrib;
	m_fields[Name].m_position = ReadPoint(archive);
	m_fields[Other].m_position = ReadPoint(archive);
	m_fields[Ref].m_position = ReadPoint(archive);
...
void CDrawMethod::OldLoad3(CStream &archive)
{
	...
	archive >> m_Symbol;
	archive >> m_fields[Name].m_value >> m_fields[Other].m_value >> m_fields[Ref].m_value >> attrib;
	m_fields[Name].m_position = ReadPoint(archive);
	m_fields[Other].m_position = ReadPoint(archive);
	m_fields[Ref].m_position = ReadPoint(archive);
...
void CDrawMethod::Load(CStream &archive)
{
	...
	for (unsigned int i = 0; i < m_fields.size(); i++)
	{
		...
		archive >> f.m_description >> f.m_value >> f.m_show >> p;
...
double CDrawMethod::DistanceFromPoint(CDPoint p)
{
	...
	if (! ( (m_point_a.x < p.x - 10 && m_point_b.x < p.x - 10) || (m_point_a.x > p.x + 10 && m_point_b.x > p.x + 10) || (m_point_a.y < p.y - 10 && m_point_b.y < p.y - 10) || (m_point_a.y > p.y + 10 && m_point_b.y > p.y + 10)))
	{
...
BOOL CDrawMethod::IsInside(double left, double right, double top, double bottom)
{
	...
	if (! ( (m_point_a.x < left && m_point_b.x <= left) || (m_point_a.x > right && m_point_b.x >= right) || (m_point_a.y < top && m_point_b.y <= top) || (m_point_a.y > bottom && m_point_b.y >= bottom)))
	{
...
int CDrawMethod::IsInsideField(CDPoint p)
{
	...
	if (can_scale)
	{
		CRectTracker tracker(CRect(static_cast<int> (m_point_a.x), static_cast<int> (m_point_a.y), static_cast<int> (m_point_b.x), static_cast<int> (m_point_b.y)), CRectTracker::dottedLine | CRectTracker::resizeOutside);
	...
	if (lp < m_fields.size()) return lp + 1000;
	else return -1;
}
...
void CDrawMethod::NewRotation()
{
	...
	// Calculate the bounding box
	if ( (rotate & 3) < 2) m_point_b = CDPoint(tr.x + m_point_a.x, tr.y + m_point_a.y);
	else m_point_b = CDPoint(tr.y + m_point_a.x, tr.x + m_point_a.y);
...
void CDrawMethod::Paint(CContext &dc, paint_options options)
{
	...
	for (unsigned int lp = 0; lp < m_fields.size(); lp++)
	{
		if (m_fields[lp].m_show) dc.TextOut(GetDecoratedField(lp), CDPoint(m_fields[lp].m_position.x + m_point_a.x, m_fields[lp].m_position.y + m_point_a.y), options);
	}
...
{
	CRect r(static_cast<int> (m_point_a.x), static_cast<int> (m_point_a.y), static_cast<int> (m_point_b.x), static_cast<int> (m_point_b.y));
	r.NormalizeRect();
